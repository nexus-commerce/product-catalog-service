...
import (
	...
	productv1 "github.com/nexus-commerce/nexus-contracts-go/product/v1"
...
func (s *Service) ListProducts(ctx context.Context, searchTerm string, page, pageSize int32) ([]*model.Product, int32, error) {
	if page <= 0 {
		page = 1
	}
	if pageSize <= 0 {
...
func (s *Service) CreateProduct(ctx context.Context, p *productv1.Product) (*model.Product, error) {
	...
	_, err := s.mongoRepository.CreateProduct(ctx, product)
	if err != nil {
		return nil, err
	}
...
func (s *Service) UpdateProduct(ctx context.Context, r *productv1.UpdateProductRequest) (*model.Product, error) {
	product, err := s.mongoRepository.GetProductByID(ctx, r.GetId())
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			return nil, ErrNotFound
		}
	...
	if r.GetUpdateMask() != nil {
		for _, path := range r.GetUpdateMask().GetPaths() {
			...
			case "image_url":
				product.ImageURL = r.Product.GetImageUrl()
			case "attributes":
				product.Attributes = r.Product.GetAttributes()
			}
		}
	}
